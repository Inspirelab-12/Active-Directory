...
Special Thanks  : SpiceHeads JeffLatham01 and Onelinez for their suggestions for improvement and for testing.
ChangeLog       :    Date     Who  Description of changes
		  ----------  ---  ----------------------------------------------------------------------------
                  ...
                  10/23/2018  RMW  Removed all specific error handling catch routines and just have one.
                  10/31/2018  RMW  Added more code for catching errors
                  11/15/2018  RMW  Server list file changed to .csv to allow Descriptions for the servers.
                  09/10/2019  RMW  Added an Overview section
                  12/31/2019  RMW  Added CPU Average Utilization %
		  ----------  ---  ----------------------------------------------------------------------------
           Intro: This PowerShell script produces a report that shows drive, RAM, and CPU usage as well as uptime
                  for a list of servers fed to it in a .csv file.
                  You can have the report include an "Overview" section that lists any of the items noted above
                  if they are in a warning or critical state. You determine the values to identify those states.
                  For example, setting the "$Warning" variable to 15 will trigger the report to highlight any drive
                  that is below 15% available space to be color coded in the color you designate in $BGColorWarn.
                  You can also disable the "Overview" section entirely if it is of no value to you.
                  Variable definitions are described below with the section below that for setting your preferences.
                  The server list file contains the server name and description, one per line, comma separated, as
                  shown below:
                  Server1,Web Server
=====================================================================================================================
...
$ErrorActionPreference = Set default error action to 'Stop' for error handling
      ...
      $EmailAttachment = Email the report as an attachment? Yes or No - will aslo be included in the HTML body
       ...
       $OverviewOption = Do you want to include the Overvew section on the report? Yes or No
        $global:Alerts = Global counter for tracking the number of alerts
======================================================================================================================#>
